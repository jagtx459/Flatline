...
const MAX_RESTORE_WAIT_SECONDS = 3600;
...
function parseInfraConfig(kind, raw) {
  ...
  switch (kind) {
    ...
    case 'http': {
      ...
      cfg.method ??= 'POST';
      if (cfg.auth_scheme && !AUTH_SCHEMES.includes(cfg.auth_scheme)) {
        return `auth_scheme must be one of ${AUTH_SCHEMES.join('/')}`;
      }
...
function parseRelayConfig(kind, raw) {
  const src = typeof raw === 'object' && raw !== null ? raw : {};
  const cfg = {};
 ...
 */
function parseRelayNetwork(raw) {
  ...
  if (parts[1] === 'actions' && parts[2] === 'targets' && parts.length === 5 && parts[4] === 'restore' && method === 'POST') {
    ...
    if (target.kind === 'ssh' || target.kind === 'winrm') {
      ...
      sendJson(res, 202, {
        ...
        message: `Restore sequence started for ${target.name}. It can take a few minutes — the result appears in this target's last activity.`
      });
      return;
    }
